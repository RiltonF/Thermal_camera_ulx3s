...
class OV7670:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        Args:
            ...
            reset (Optional[microcontroller.Pin]): If not None, the eset signal
                to the camera.
    ...
    def _write_register(self, reg: int, value: int) -> None:
        print(f"writing to reg: {reg:#x}, of value: {value:#b}")
        self.config_dump.append((((reg & 0xFF)<< 8) | (value & 0xFF)) & 0xFFFF)
        b = bytearray(2)
        b[0] = reg
        b[1] = value
    ...
    def _read_register(self, reg: int) -> int:
        ...
        with self._i2c_device as i2c:
            i2c.write(b)
            i2c.readinto(b)
        print(f"reading reg: {reg:#x}, result:{b[0]:#b}")
        return b[0]
